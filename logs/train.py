...
def process_data(file_path, model_name_suffix="", shuffle=True):
    # Load the data
    df = pd.read_csv(file_path)

    # shuffle or not the data
    if shuffle:
    ...
    evaluate_model(y_test, rf_pred, f"Random Forest {model_name_suffix}")
    evaluate_model(y_test, lr_pred, f"Logistic Regression {model_name_suffix}")
...
# Process the new 10-minute interval data
process_data('BGL_template_counts_error_prediction.csv', model_name_suffix="ERROR PREDICTION SHUFFLED", shuffle=True)
...
# Process the original 30-minute interval data
process_data('BGL_template_counts_error_detection.csv', model_name_suffix="ERROR DETECTION SHUFFLED", shuffle=True)